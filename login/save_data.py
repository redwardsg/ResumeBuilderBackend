...
def save_section():
    if request.method == 'POST':
        ...
        values_personal = (value,firstname, lastname, str(date_obj))
...
#         return jsonify({'alternate_email': var_1[1], 'phone_number': var_1[2],'alternate_number': var_1[3],'linkedin_url': var_1[4]})
        ...
        #for workExperience in data['workExpirenceInfo']:
        for work in data['workExpirenceInfo']:
            company_name = work['companyName']
            position = work['position']
            start_date = work['date']
            
            start_date1 = datetime.strptime(start_date, "%Y-%m-%d").date()
            current_job = work['currentJob']
